...
class InfoReportesController extends AbstractController
{
    ...
    public function excel(Request $objRequest)
    {
        ...
        try
        {
            ...
            $mesActual  = strtoupper($meses[$intMes]); // Traducir y convertir a mayúsculas
            $anioActual = $intAnio; //date('Y'); // Obtener el año actual
            ...
            foreach ($groupedData as $area => $items) {
                ...
                foreach ($items as $item) {
                    $row = array_map(function($key) use ($item) {
                        return $item[$key];
                    }, $headers);
            ...
            $maxLength = max(array_map(function($items) {
                return count($items);
            }, $groupedData));
            ...
            foreach ($areas as $area) {
                $areaHeaders = array_filter(
                    array_keys($groupedData[$area][0]),
                    function($key) use ($excludedKeys) {
                        return !in_array($key, $excludedKeys);
                    }
                );
            ...
            $horizontalHeaders = array_unique(
                array_filter($horizontalHeaders, function ($value) {
                    return trim($value) !== '';
                })
            );
